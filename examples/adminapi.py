...
def example_list_consumer_groups(a, args):
    ...
    if len(args) > 0:
        ...
        for i in range(0,len(args)):
            if (args[i] == "-states"):
                if (isState):
                    raise Exception("Invalid Arguments \n Usage : list_consumer_groups [-states <state1> <state2> ..] [-types <grouptype1> <grouptype2> ..]\n")
                isState = True
            elif (args[i] == "-types"):
                if (isType):
                    raise Exception("Invalid Arguments \n Usage : list_consumer_groups [-states <state1> <state2> ..] [-types <grouptype1> <grouptype2> ..]\n")
                ...
                if (isType):
                    group_types = group_types.add(getConsumerGroupType(args[i]))
                elif (isState):
                    states = states.add(getConsumerGroupState(args[i]))
                else:
                    raise Exception("Invalid Arguments \n Usage : list_consumer_groups [-states <state1> <state2> ..] [-types <grouptype1> <grouptype2> ..]\n")
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        ...
        sys.stderr.write(' list_consumer_groups [-states <state1> <state2> ..] [-types <grouptype1> <grouptype2> ..]\n')

...
"""KIP-932 share consumer demo, commit_sync variant.

Same processing as demo_share_consumer.py, but commits synchronously after
every non-empty batch and prints the per-partition result dict. Useful when
you want to see commit landing inline; costs one broker round-trip per batch.

Usage: python examples/demo_share_consumer_commit_sync.py <NAME>
"""
...
def handle(name, sc, m, counters):
    ...
    # Application dependent message handling logic goes here. In this demo we just sleep a bit to simulate work, then ACK each message with a type based on its value. 
...
def main():
    ...
    print(f'[{name}] subscribed; commit_sync per batch (Ctrl+C to stop)', flush=True)
    try:
        while True:
            # Poll for a batch of messages, handle them, then commit synchronously to see results inline.
            batch = sc.poll(timeout=1.0)

            # Handle messages in batch, acknowledging each one.
            for m in batch:
                handle(name, sc, m, counters) # process + ack apis

            # Commit synchronously after every non-empty batch, to see commit results inline.
            if batch:
                result = sc.commit_sync(timeout=5.0)
                print(f'{CYAN}Commit Sync Response: {format_commit_result(result)}{RESET}', flush=True)
    except KeyboardInterrupt:
        print(f'\n[{name}] final flush...', flush=True)
        try:
            result = sc.commit_sync(timeout=5.0)
            print(f'{CYAN}Commit Sync Response: {format_commit_result(result)}{RESET}', flush=True)

...
"""KIP-932 share consumer demo, explicit-ack mode.

Pair with demo_producer.py. Run multiple instances in parallel with
different NAME arguments to see records distributed across share-group members.

Usage: python examples/demo_share_consumer.py <NAME>
"""
...
def handle(name, sc, m, counters):
    ...
    if value.startswith('msg-reject'):
        ...
        # Keep releasing; broker archives once delivery_count hits delivery.attempt.limit.
...
def main():
    ...
    print(f'[{name}] subscribed; waiting for records (Ctrl+C to stop)', flush=True)
    try:
        while True:
        ...
        print(f'\n[{name}] flushing pending acks via commit_sync...', flush=True)
        try:

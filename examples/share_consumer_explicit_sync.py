...
def main():
    ...
    print(f"[setup] group={group} mode=explicit commit=sync reset={args.reset} topic={args.topic}")
    ...
    try:
        while True:
            ...
            for tp, err in sorted(sc.commit_sync().items(),
                ...
                print(f"[commit-sync] partition={tp.partition} -> {'ok' if err is None else err}")

...
if [[ -n "${MULTI_REPLICAS:-}" ]] && [[ "${SHARE:-}" == "true" || "${EXPLICIT:-}" == "true" || "${PERF:-}" == "true" ]]; then
    echo "ERROR: MULTI_REPLICAS is not compatible with SHARE/EXPLICIT/PERF" >&2
    exit 1
fi

if [[ -n "${MULTI_REPLICAS:-}" ]]; then
    # soak_multi.py runs MULTI_REPLICAS full producer+consumer replicas in
    # one free-threaded process
    client_script="soak_multi.py"
    client_args="-i $TESTID -t $topic -r 80 -f $1 --num-replicas $MULTI_REPLICAS"
else
    client_script="soakclient.py"
    client_args="-i $TESTID -t $topic -r 80 -f $1 $share_flag $explicit_flag $perf_flag"
fi

echo "Starting soak client ($client_script) using topic $topic. Logging to $logfile."
set +x
while [ "$run" = true ]; do
    # Ignore SIGINT in children (inherited)
    trap "" SIGINT
    time opentelemetry-instrument $testdir/$client_script $client_args |& tee /dev/tty | bzip2 > $logfile &
    PID=$!
    terminate_last() {
        # List children of $PID only
        ps --ppid $PID -f | grep $client_script | grep -v grep | awk '{print $2}' | xargs kill
    }

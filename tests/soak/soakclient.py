...
class SoakClient(object):
    ...
    def dr_cb(self, err, msg):
        ...
        if err is not None:
            ...
            self.incr_counter(
                ...
                1,
                {"topic": msg.topic(), "partition": str(msg.partition()), "err": str(err)},
            )
    ...
    def producer_status(self):
        ...
        self.logger.info(
            "producer: {} messages produced, {} delivered, {} failed, {} error_cbs".format(
                self.producer_msgid, self.dr_cnt, self.dr_err_cnt, self.producer_error_cb_cnt
            )
        )
    ...
    def consumer_status(self):
        ...
        self.logger.info(
            "consumer: {} messages consumed, {} duplicates, "
            "{} missed, {} message errors, {} consumer errors, {} error_cbs".format(
                self.msg_cnt,
                self.msg_dup_cnt,
                self.msg_miss_cnt,
                self.msg_err_cnt,
                self.consumer_err_cnt,
    ...
    def consumer_run(self):
        ...
        while self.run:
            ...
            try:
                ...
                self.logger.info(
                    "consumer: Failed to deserialize message in "
            ...
            if (self.msg_cnt % self.disprate) == 0:
                ...
                self.logger.info(
                    "consumer: {} messages consumed: Message {} "
                    "[{}] at offset {} (headers {}, latency {})".format(
                        self.msg_cnt, msg.topic(), msg.partition(), msg.offset(), msg.headers(), latency
                    )
                )
            ...
            if hw > 0:
                if msg.offset() <= hw:
                    self.logger.warning(
                        "consumer: Old or duplicate message {} "
                        "[{}] at offset {} (headers {}): wanted offset > {} (last commited {})".format(
                            msg.topic(), msg.partition(), msg.offset(), msg.headers(), hw, self.last_committed
                        )
                    )
    ...
    def rtt_stats(self, d):
        ...
        for broker in d['brokers'].values():
            ...
            tags = {
                "broker": "{}".format(broker['nodeid']),
                "partitions": "{}".format(parts),
                "type": "{}".format(d['type']),
            }

            self.set_gauge("broker.rtt.p99", float(broker['rtt']['p99']) / 1000000.0, tags=tags)
            self.set_gauge("broker.rtt.avg", float(broker['rtt']['avg']) / 1000000.0, tags=tags)
    ...
    def stats_cb(self, json_str):
        ...
        if self.topic in d['topics']:
            leaders = [
                '{}={}'.format(p['partition'], p['leader'])
                for p in d['topics'][self.topic]['partitions'].values()
                if p['partition'] != -1
            ]
        else:
            leaders = []
        self.logger.info(
            "{} stats: {}/{} brokers UP, {} partition leaders: {}".format(
    ...
    def __init__(self, testid, topic, rate, conf):
        ...
        if 'group.id' not in conf:
            ...
            conf['group.id'] = 'soakclient-{}-{}-{}'.format(self.hostname, version(), sys.version.split(' ')[0])
    ...
    def incr_counter(self, metric_name, incrval, tags=None):
        ...
        if not tags:
            tags = {}
        tags.update({"host": self.hostname, "testid": self.testid})
    ...
    def set_gauge(self, metric_name, val, tags=None):
        ...
        if full_metric_name not in self.gauges:
            ...
            self.gauges[full_metric_name] = self.meter.create_observable_gauge(
                callbacks=[self.gauge_cbs[full_metric_name]], name=full_metric_name, description=full_metric_name
            )
    ...
    def calc_rusage_deltas(self, curr, prev, elapsed):
        ...
        self.logger.info("User CPU: {:.1f}%, System CPU: {:.1f}%, MaxRSS {:.3f}MiB".format(user_cpu, sys_cpu, max_rss))

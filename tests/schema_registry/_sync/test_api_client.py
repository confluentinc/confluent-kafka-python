...
def test_register_schema_cache(mock_schema_registry, load_avsc):
    ...
    def register():
        return sr.register_schema('test-cache', schema)
...
def test_delete_subject_invalidates_cached_reads(respx_mock, permanent, cache_ttl, method, args, verb, suffix):
    sr = SchemaRegistryClient({'url': TEST_URL, 'cache.latest.ttl.sec': cache_ttl})
    ...
    for subject in ('deleted-subject', 'other-subject'):
        ...
        assert (read(subject, *args)).schema_id == 47
    ...
    assert sr.delete_subject('deleted-subject', permanent=permanent) == [1]

    with pytest.raises(SchemaRegistryError, match='Subject not found'):
        read('deleted-subject', *args)
    assert routes['deleted-subject'].call_count == 2
    assert (read('other-subject', *args)).schema_id == 47
    assert routes['other-subject'].call_count == 1


@pytest.mark.parametrize('permanent', [False, True])
@pytest.mark.parametrize('delete_succeeds', [False, True])
def test_register_schema_after_subject_deletion(respx_mock, permanent, delete_succeeds):
    ...
    assert sr.register_schema('test-delete', schema) == 47
    assert sr.register_schema('test-delete', schema) == 47
...
def test_latest_read_during_invalidation_is_not_cached(monkeypatch, method, args, invalidate):
    sr = SchemaRegistryClient({'url': TEST_URL})
    calls = 0
    ...
    def get(*args, **kwargs):
        ...
        getattr(sr, invalidate)(*invalidate_args)
    ...
    read = getattr(sr, method)
    assert (read('test-delete', *args)).schema_id == 47
    with pytest.raises(SchemaRegistryError, match='Subject not found'):
        read('test-delete', *args)
    assert calls == 2
...
def test_subject_read_during_invalidation_is_not_cached(monkeypatch, method, args, invalidate, invalidate_args):
    sr = SchemaRegistryClient({'url': TEST_URL})
    calls = 0
...
def test_delete_subject_when_metadata_expires_during_invalidation(respx_mock):
    clock = [0]
    ...
    assert sr.delete_subject('test-delete') == [1]
...
def test_send_request_exhausts_retries_on_network_error():
    ...
    req = httpx.Request('GET', TEST_URL + '/subjects')
    mock_request = Mock(side_effect=httpx.ConnectError('connection refused', request=req))
    rest_client.session.request = mock_request

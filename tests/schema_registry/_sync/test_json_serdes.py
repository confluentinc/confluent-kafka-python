...
def test_json_cel_field_transform_all_of():
    conf = {'url': _BASE_URL}
    client = SchemaRegistryClient.new_client(conf)
    ser_conf = {'auto.register.schemas': False, 'use.latest.version': True}
    schema = {
...
def test_json_cel_field_transform_nested_any_of():
    conf = {'url': _BASE_URL}
    client = SchemaRegistryClient.new_client(conf)
    ser_conf = {'auto.register.schemas': False, 'use.latest.version': True}
    schema = {
...
def test_json_cel_field_transform_sibling_any_of():
    conf = {'url': _BASE_URL}
    client = SchemaRegistryClient.new_client(conf)
    ser_conf = {'auto.register.schemas': False, 'use.latest.version': True}
    schema = {
...
def test_json_payloadencryption():
    ...
    client.register_schema(_SUBJECT, Schema(json.dumps(schema), "JSON", [], None, RuleSet(None, None, [rule])))
...
def test_json_encryption_with_references():
    ...
    client.register_schema(_SUBJECT, Schema(json.dumps(schema), "JSON", refs, None, RuleSet(None, [rule])))
...
def test_json_oneof_with_refs_nested_refs():
    """
...
def test_json_deeply_nested_refs():
    """
...
def test_json_associated_name_strategy_with_association():
    ...
    ser_ctx = SerializationContext(_TOPIC, MessageField.VALUE)
    obj_bytes = ser(_JSON_OBJ, ser_ctx)
...
def test_json_associated_name_strategy_fallback_to_record():
...
def test_json_associated_name_strategy_fallback_to_topic_record():
...
def test_json_associated_name_strategy_fallback_none_raises():
...
def test_json_associated_name_strategy_caching():
    ...
    registered_schema = client.get_latest_version("json-cached-subject")
    assert registered_schema is not None

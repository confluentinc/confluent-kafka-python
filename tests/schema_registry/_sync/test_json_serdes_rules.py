...
#
"""
Tests for JSON serdes that depend on the rules extra (tink/celpy) and are
...
from tests.schema_registry._sync.test_avro_serdes_rules import FakeClock
...
def test_json_nullable_array_of_refs():
    """
    ...
    # never encrypt.
    obj_bytes = ser(obj_with_array, ser_ctx)
    assert obj_with_array['investments'][0]['amount'] != '1000'

    deser = JSONDeserializer(None, schema_registry_client=client, rule_conf=rule_conf)
    executor.executor.client = dek_client
    decoded = deser(obj_bytes, ser_ctx)
    ...
    executor.executor.client = dek_client
    obj_bytes_null = ser(obj_with_null, ser_ctx)
    executor.executor.client = dek_client
    decoded_null = deser(obj_bytes_null, ser_ctx)

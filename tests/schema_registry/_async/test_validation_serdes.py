...
from confluent_kafka.schema_registry.protobuf import AsyncProtobufSerializer
...
async def run_before_and_after_tests(tmpdir):
    ...
    for subject in await client.get_subjects():
        try:
...
def client():
    return AsyncSchemaRegistryClient.new_client({'url': _BASE_URL})
...
async def avro_serializer(**conf):
    return await AsyncAvroSerializer(client(), json.dumps(_AVRO_SCHEMA), conf=conf)


async def test_avro_serialization_passes_when_all_rules_pass():
    ser = await avro_serializer(**{'validation.rules.execution': 'AFTER_DOMAIN_RULES'})
    assert await ser({"age": 30, "name": "Alice"}, ser_ctx()) is not None


async def test_avro_serialization_passes_when_validation_disabled():
    # age=-5 would fail agePositive, but validation is disabled by default.
    ser = await avro_serializer()
    assert await ser({"age": -5, "name": "Alice"}, ser_ctx()) is not None


async def test_avro_serialization_fails_when_field_rule_fails():
    ser = await avro_serializer(**{'validation.rules.execution': 'AFTER_DOMAIN_RULES'})
...
async def test_avro_serialization_fails_when_record_rule_fails():
    ser = await avro_serializer(**{'validation.rules.execution': 'AFTER_DOMAIN_RULES'})
    with pytest.raises(SerializationError) as exc:
        await ser({"age": 200, "name": "Alice"}, ser_ctx())
    assert "ageNotInsane" in str(exc.value)


async def test_avro_serialization_reports_every_violation():
    ser = await avro_serializer(**{'validation.rules.execution': 'AFTER_DOMAIN_RULES'})
    with pytest.raises(SerializationError) as exc:
        await ser({"age": -5, "name": ""}, ser_ctx())
...
async def test_avro_fail_fast_reports_a_single_violation():
    ser = await avro_serializer(
        **{'validation.rules.execution': 'AFTER_DOMAIN_RULES', 'validation.rules.fail.fast': True}
    )
    with pytest.raises(SerializationError) as exc:
        await ser({"age": -5, "name": ""}, ser_ctx())
    message = str(exc.value)
...
async def test_avro_validation_runs_with_use_latest_version():
    # The registry-resolved schema path, as opposed to the serializer's own schema.
    sr = client()
    await sr.register_schema(_SUBJECT, Schema(json.dumps(_AVRO_SCHEMA), "AVRO"))
...
async def test_avro_explicit_executor_is_used():
    ser = await avro_serializer(
...
async def test_invalid_validation_config_is_rejected(conf, match):
    with pytest.raises(ValueError, match=match):
        await avro_serializer(**conf)
...
async def json_serializer(**conf):
    return await AsyncJSONSerializer(json.dumps(_JSON_SCHEMA), client(), conf=conf)


async def test_json_serialization_passes_when_all_rules_pass():
    ser = await json_serializer(**{'validation.rules.execution': 'AFTER_DOMAIN_RULES'})
    assert await ser({"age": 30, "name": "Alice"}, ser_ctx()) is not None


async def test_json_serialization_passes_when_validation_disabled():
    ser = await json_serializer()
    assert await ser({"age": -5, "name": "Alice"}, ser_ctx()) is not None


async def test_json_serialization_fails_when_field_rule_fails():
    ser = await json_serializer(**{'validation.rules.execution': 'AFTER_DOMAIN_RULES'})
    with pytest.raises(SerializationError) as exc:
        await ser({"age": -5, "name": "Alice"}, ser_ctx())
    assert "agePositive" in str(exc.value)
...
async def test_json_serialization_reports_every_violation():
    ser = await json_serializer(**{'validation.rules.execution': 'AFTER_DOMAIN_RULES'})
    with pytest.raises(SerializationError) as exc:
        await ser({"age": -5, "name": ""}, ser_ctx())
    message = str(exc.value)
...
async def test_json_validation_runs_with_use_latest_version():
    sr = client()
    await sr.register_schema(_SUBJECT, Schema(json.dumps(_JSON_SCHEMA), "JSON"))
    ser = await AsyncJSONSerializer(
...
async def protobuf_serializer(**conf):
    return await AsyncProtobufSerializer(
...
async def test_protobuf_serialization_passes_when_all_rules_pass():
    ser = await protobuf_serializer(**{'validation.rules.execution': 'AFTER_DOMAIN_RULES'})
    assert await ser(person(30, "Alice"), ser_ctx()) is not None


async def test_protobuf_serialization_passes_when_validation_disabled():
    ser = await protobuf_serializer()
    assert await ser(person(-5, "Alice"), ser_ctx()) is not None


async def test_protobuf_serialization_fails_when_field_rule_fails():
    ser = await protobuf_serializer(**{'validation.rules.execution': 'AFTER_DOMAIN_RULES'})
    with pytest.raises(SerializationError) as exc:
        await ser(person(-5, "Alice"), ser_ctx())
    message = str(exc.value)
    assert "agePositive" in message
...
async def test_protobuf_serialization_fails_when_message_rule_fails():
    ser = await protobuf_serializer(**{'validation.rules.execution': 'AFTER_DOMAIN_RULES'})
    with pytest.raises(SerializationError) as exc:
        await ser(person(200, "Alice"), ser_ctx())
    assert "ageNotInsane" in str(exc.value)


async def test_protobuf_serialization_reports_every_violation():
    ser = await protobuf_serializer(**{'validation.rules.execution': 'AFTER_DOMAIN_RULES'})
    with pytest.raises(SerializationError) as exc:
        await ser(person(200, ""), ser_ctx())
    message = str(exc.value)
    assert "2 violations" in message
...
async def test_protobuf_dynamic_failure_message_is_reported():
    ser = await AsyncProtobufSerializer(
    ...
    with pytest.raises(SerializationError) as exc:
        await ser(validation_widget_pb2.ValidationDynamicMessage(age=-5), ser_ctx())

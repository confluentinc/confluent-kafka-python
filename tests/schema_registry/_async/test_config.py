...
def test_config_url_invalid_type():
    conf = {'url': dict()}
    with pytest.raises(TypeError, match="url must be a str," " not <(.*)>$"):
        AsyncSchemaRegistryClient(conf)


def test_config_url_none():
    conf = {}
    with pytest.raises(ValueError, match="Missing required configuration" " property url"):
        AsyncSchemaRegistryClient(conf)
...
def test_config_ssl_key_no_certificate():
    ...
    with pytest.raises(
        ValueError,
        match="ssl.certificate.location required" " when configuring ssl.key.location" " or ssl.key.password",
    ):
        AsyncSchemaRegistryClient(conf)


def test_config_ssl_password_no_certificate():
    conf = {'url': TEST_URL, 'ssl.key.password': 'sesame'}
    with pytest.raises(
        ValueError,
        match="ssl.certificate.location required" " when configuring ssl.key.location" " or ssl.key.password",
    ):
...
def test_config_auth_url():
    conf = {'url': 'http://' + TEST_USERNAME + ":" + TEST_USER_PASSWORD + '@SchemaRegistry:65534'}
    test_client = AsyncSchemaRegistryClient(conf)
    assert (
        test_client._rest_client.session.auth._auth_header == BasicAuth(TEST_USERNAME, TEST_USER_PASSWORD)._auth_header
    )


def test_config_auth_url_and_userinfo():
    conf = {
        'url': 'http://' + TEST_USERNAME + ":" + TEST_USER_PASSWORD + '@SchemaRegistry:65534',
    ...
    with pytest.raises(
        ValueError,
        match="basic.auth.user.info configured with"
...
def test_config_auth_userinfo():
    conf = {'url': TEST_URL, 'basic.auth.user.info': TEST_USERNAME + ':' + TEST_USER_PASSWORD}
...
def test_config_auth_userinfo_invalid():
    conf = {'url': TEST_URL, 'basic.auth.user.info': 'lookmanocolon'}

    with pytest.raises(ValueError, match="basic.auth.user.info must be in the" " form of {username}:{password}$"):
        AsyncSchemaRegistryClient(conf)

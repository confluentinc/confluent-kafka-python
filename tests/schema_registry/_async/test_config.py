...
def test_oauth_bearer_azure_imds_config_invalid():
    ...
    for url in [{'bearer.auth.issuer.endpoint.url': 'http://test'}, {}]:
        ...
        conf = {'url': TEST_URL,
                ...
                'bearer.auth.logical.cluster': TEST_CLUSTER,
                'bearer.auth.identity.pool.id': TEST_POOL,
                'bearer.auth.issuer.endpoint.query': 'test=1',
                **url}

        with pytest.raises(ValueError, match=r"Missing required 'api-version' query parameter in token endpoint URL"):
            AsyncSchemaRegistryClient(conf)
        ...
        conf = {'url': TEST_URL,
                ...
                'bearer.auth.logical.cluster': TEST_CLUSTER,
                'bearer.auth.identity.pool.id': TEST_POOL,
                'bearer.auth.issuer.endpoint.query': 'test=1&api-version=2018-02-01',
                **url}

        with pytest.raises(ValueError, match=r"Missing required 'resource' query parameter in token endpoint URL"):
            AsyncSchemaRegistryClient(conf)

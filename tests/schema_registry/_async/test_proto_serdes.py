...
async def test_proto_cel_decimal_passes():
    conf = {'url': _BASE_URL}
    client = AsyncSchemaRegistryClient.new_client(conf)
    ser_conf = {'auto.register.schemas': False, 'use.latest.version': True, 'use.deprecated.format': False}
    rule = Rule(
...
async def test_proto_cel_decimal_fails():
    conf = {'url': _BASE_URL}
    client = AsyncSchemaRegistryClient.new_client(conf)
    ser_conf = {'auto.register.schemas': False, 'use.latest.version': True, 'use.deprecated.format': False}
    rule = Rule(
...
async def test_proto_cel_decimal_arithmetic():
    conf = {'url': _BASE_URL}
    client = AsyncSchemaRegistryClient.new_client(conf)
    ser_conf = {'auto.register.schemas': False, 'use.latest.version': True, 'use.deprecated.format': False}
    rule = Rule(
...
async def test_proto_cel_decimal_mod():
    conf = {'url': _BASE_URL}
    client = AsyncSchemaRegistryClient.new_client(conf)
    ser_conf = {'auto.register.schemas': False, 'use.latest.version': True, 'use.deprecated.format': False}
    rule = Rule(
...
async def test_proto_cel_decimal_greatest_least():
    conf = {'url': _BASE_URL}
    client = AsyncSchemaRegistryClient.new_client(conf)
    ser_conf = {'auto.register.schemas': False, 'use.latest.version': True, 'use.deprecated.format': False}
    rule = Rule(
...
async def test_proto_cel_decimal_sqrt():
    conf = {'url': _BASE_URL}
    client = AsyncSchemaRegistryClient.new_client(conf)
    ser_conf = {'auto.register.schemas': False, 'use.latest.version': True, 'use.deprecated.format': False}
    rule = Rule(
...
async def test_proto_cel_timestamp_passes():
    ...
    ser = await AsyncProtobufSerializer(nested_pb2.NestedMessage, client, conf=ser_conf)
    ser_ctx = SerializationContext(_TOPIC, MessageField.VALUE)
    obj_bytes = await ser(obj, ser_ctx)

    deser_conf = {'use.deprecated.format': False}
    deser = await AsyncProtobufDeserializer(nested_pb2.NestedMessage, deser_conf, client)
    obj2 = await deser(obj_bytes, ser_ctx)
    assert obj == obj2


async def test_proto_cel_timestamp_fails():
    conf = {'url': _BASE_URL}
    client = AsyncSchemaRegistryClient.new_client(conf)
    ser_conf = {'auto.register.schemas': False, 'use.latest.version': True, 'use.deprecated.format': False}
    rule = Rule(

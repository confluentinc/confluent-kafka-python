...
def test_subscribe_empty_list_clears_subscription(share_consumer):
    """subscribe([]) acts as unsubscribe: it clears any existing subscription,
    after which poll() raises _STATE (not subscribed).

    This is an empty *list* — an empty topic *name* is a different case and
    still raises _INVALID_ARG.
    """
    share_consumer.subscribe(['test-topic'])

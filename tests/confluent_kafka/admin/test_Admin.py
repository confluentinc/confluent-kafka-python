...
def test_types():
    ConfigResource(RESOURCE_BROKER, "2")
    ConfigResource("broker", "2")
    ConfigResource(RESOURCE_GROUP, "mygroup")
    ConfigResource(RESOURCE_TOPIC, "")
    with pytest.raises(ValueError):
        ConfigResource("doesnt exist", "hi")
    with pytest.raises(ValueError):
        ConfigResource(RESOURCE_TOPIC, None)
...
def test_describe_configs_api():
    ...
    a = AdminClient({"socket.timeout.ms": 10})
    fs = a.describe_configs([ConfigResource(RESOURCE_BROKER, "3")])
    # ignore the result
    ...
    with pytest.raises(ValueError):
        a.describe_configs([None, ConfigResource(RESOURCE_TOPIC, "mytopic")])

    fs = a.describe_configs([ConfigResource(RESOURCE_TOPIC, "mytopic"),
                             ConfigResource(RESOURCE_GROUP, "mygroup")],
...
def test_alter_configs_api():
    ...
    a = AdminClient({"socket.timeout.ms": 10})
    fs = a.alter_configs([ConfigResource(RESOURCE_BROKER, "3",

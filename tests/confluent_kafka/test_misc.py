...
from confluent_kafka import Consumer, KafkaError, Producer, libversion, ThrottleEvent, version
from confluent_kafka.admin import AdminClient


def test_version():
    print('Using confluent_kafka module version %s (0x%x)' % version())
    sver, iver = version()
    assert len(sver) > 0
    assert iver > 0

    print('Using librdkafka version %s (0x%x)' % libversion())
    sver, iver = libversion()
    assert len(sver) > 0
    assert iver > 0

    assert version()[0] == confluent_kafka.__version__
...
def test_error_cb():
    ...
    def error_cb(error_msg):
        ...
        acceptable_error_codes = (
            KafkaError._TRANSPORT, KafkaError._ALL_BROKERS_DOWN)
...
def throttle_cb_instantiate():
    ...
    Producer({'throttle_cb': throttle_cb})


def test_throttle_event_types():
    throttle_event = ThrottleEvent("broker", 0, 10.0)
...
def skip_interceptors():
    ...
    for path in ["/usr/lib", "/usr/local/lib", "staging/libs", "."]:
        for ext in [".so", ".dylib", ".dll"]:
            ...
            if os.path.exists(f):
                print("interceptors at {}".format(f))
                return False
    return True

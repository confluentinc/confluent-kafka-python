...
from confluent_kafka.schema_registry.avro import AsyncAvroDeserializerBuilder, AsyncAvroSerializerBuilder
...
async def _associate(sr, topic, namespace, subject, resource_id):
    """
    ...
    try:
        await sr.register_schema(subject, Schema(SCHEMA_STR, "AVRO"))
        await sr.create_association(request)
...
async def _consume(consumer, topic, count):
...
async def test_builder_serdes_use_the_association_of_the_connected_cluster(kafka_cluster, monkeypatch):
    ...
    await _associate(sr, topic, cluster_id, subject, resource_id)

    try:
        producer = await kafka_cluster.async_builder_producer(
            {
        ...
        await producer.produce(topic, key='k1', value=RECORDS[0])
        # the first message resolved the id (subject cache miss) ...
        assert calls == [CLUSTER_ID_TIMEOUT]
        await producer.produce(topic, key='k2', value=RECORDS[1])
        ...
        subjects = await sr.get_subjects()
        assert subject in subjects
        assert topic + "-value" not in subjects

        consumer = await kafka_cluster.async_builder_consumer(
            {
                'key.deserializer': StringDeserializer('utf_8'),
                'value.deserializer.builder': AsyncAvroDeserializerBuilder(schema_registry_client=sr),
            }
        )
        try:
            assert await _consume(consumer, topic, len(RECORDS)) == RECORDS
        finally:
            await consumer.close()
    finally:
        await sr.delete_associations(resource_id=resource_id, cascade_lifecycle=True)


async def test_association_under_wildcard_namespace_is_not_seen_by_a_connected_client(kafka_cluster):
    ...
    await _associate(sr, topic, "-", subject, resource_id)
    ...
    try:
        ...
        try:
            await producer.produce(topic, value=RECORDS[0])
            await producer.flush()
        finally:
            await producer.close()

        assert topic + "-value" in await sr.get_subjects()
    finally:
        await sr.delete_associations(resource_id=resource_id, cascade_lifecycle=True)
        # a subject has to be soft-deleted before it can be deleted permanently
        await sr.delete_subject(topic + "-value")
        await sr.delete_subject(topic + "-value", permanent=True)

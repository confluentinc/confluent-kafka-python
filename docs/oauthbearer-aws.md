...
This adds `boto3` and optionally other third party dependencies to the dependency graph. Without the extra, nothing more is imported or installed.
...
- **Both SASL settings are required.** Set `security.protocol=SASL_SSL` and `sasl.mechanisms=OAUTHBEARER`; without them the OAUTHBEARER token-refresh path never engages and authentication silently does not happen.
...
- `boto3[crt]` **may be needed for** `aws login` **credential sessions.** Like any boto3 application, this integration uses boto3's default credential chain — so if your active AWS profile uses an `aws login` session (a `login_session` entry), boto3 raises a `MissingDependencyException` for a missing CRT dependency. This does not occur when credentials come from an attached IAM role (as on AWS compute). Fix: add an additional `boto3[crt]` to your requirements, or use a credential source without a login session.

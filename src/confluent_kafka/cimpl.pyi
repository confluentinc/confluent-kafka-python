...
import builtins
from typing import Any, Callable, Dict, Generic, List, Literal, Optional, Set, Tuple, Union, overload
...
# PEP 696 defaults land in typing on Python 3.13; import unconditionally from
# typing_extensions, which type checkers always have available. This file is a
# stub and is never executed, so there is no runtime dependency either way.
from typing_extensions import TypeVar

from confluent_kafka.admin._metadata import ClusterMetadata, GroupMetadata

#: Deserialized message key and value types. They default to ``bytes``, which is
#: what the raw clients hand back, so an unparameterized ``Message`` keeps the
#: typing it has always had. :py:class:`confluent_kafka.DeserializingConsumer`
#: parameterizes them with its deserializers' output types.
K = TypeVar("K", default=bytes)
V = TypeVar("V", default=bytes)
...
class Message(Generic[K, V]):
    ...
    # Same underlying slots as key()/value(). On a message from a
    # DeserializingConsumer these hold the deserialized objects; elsewhere they
    # hold the raw payload, which is why K/V default to bytes.
    def deserialized_key(self) -> Optional[K]: ...
    def deserialized_value(self) -> Optional[V]: ...
...
class Producer:
    ...
    def cluster_id(self, timeout: float = -1) -> str: ...

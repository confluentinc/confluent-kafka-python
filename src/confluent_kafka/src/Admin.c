...
Admin_list_consumer_groups(Handle *self, PyObject *args, PyObject *kwargs) {
        ...
        static char *kws[] = {"future",
                              /* options */
                              "states_int", "request_timeout", NULL};

        if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O|Of", kws, &future,
                                         &states_int,
                                         &options.request_timeout)) {
                goto err;
        }
                ...
                if (states_cnt > 0) {
                        ...
                        for (i = 0; i < states_cnt; i++) {
                                ...
                                if (!cfl_PyInt_Check(state)) {
                                        PyErr_SetString(
                                            PyExc_ValueError,
                                            "Element of states must be a valid "
                                            "state");
                                        goto err;
                                }
...
const char Admin_list_consumer_groups_doc[] = PyDoc_STR(
    ".. py:function:: list_consumer_groups(future, [states_int], "
    "[request_timeout])\n"
    "\n"
...
raise:
        ...
        exc = ((PyTypeObject *)exctype)
                  ->tp_new((PyTypeObject *)exctype, NULL, NULL);

...
static PyObject *KafkaError_txn_requires_abort(KafkaError *self,
                                               PyObject *ignore) {
...
static int KafkaError_traverse(KafkaError *self, visitproc visit, void *arg) {
        return 0;
}

static PyObject *KafkaError_str0(KafkaError *self) {
        return cfl_PyUnistr(_FromFormat(
            "KafkaError{%scode=%s,val=%d,str=\"%s\"}",
            self->fatal ? "FATAL," : "", rd_kafka_err2name(self->code),
            self->code, self->str ? self->str : rd_kafka_err2str(self->code)));
}
...
static PyObject *
KafkaError_richcompare(KafkaError *self, PyObject *o2, int op) {
        int code2;
        int r;
        PyObject *result;
...
static PyObject *
KafkaError_new(PyTypeObject *type, PyObject *args, PyObject *kwargs) {
 ...
 */
static void
KafkaError_init(KafkaError *self, rd_kafka_resp_err_t code, const char *str) {
        self->code               = code;
        self->fatal              = 0;
        self->retriable          = 0;
...
PyObject *KafkaError_new0(rd_kafka_resp_err_t err, const char *fmt, ...) {
        ...
        self = (KafkaError *)KafkaErrorType.tp_alloc(&KafkaErrorType, 0);
        if (!self)
                return NULL;
...
static PyObject *Message_timestamp(Message *self, PyObject *ignore) {
        return Py_BuildValue("iL", self->tstype, self->timestamp);
}
...
static int Message_init(PyObject *self0, PyObject *args, PyObject *kwargs) {
        ...
        if (timestamp && timestamp != Py_None) {
                ...
                self->tstype = (rd_kafka_timestamp_type_t)cfl_PyInt_AsInt(
                    PyTuple_GET_ITEM(timestamp, 0));
                self->timestamp =
                    PyLong_AsLongLong(PyTuple_GET_ITEM(timestamp, 1));
        } else {
...
static int Message_traverse(Message *self, visitproc visit, void *arg) {
        if (self->topic)
                Py_VISIT(self->topic);
        if (self->value)
                Py_VISIT(self->value);
...
PyObject *Message_new0(const Handle *handle, const rd_kafka_message_t *rkm) {
        ...
        self->error = KafkaError_new_or_None(
            rkm->err, (rkm->err && handle->type != RD_KAFKA_PRODUCER)
        ...
        if (rkm->key)
                self->key =
                    cfl_PyBin(_FromStringAndSize(rkm->key, rkm->key_len));
...
static PyMethodDef Uuid_methods[] = {
    {"get_most_significant_bits", (PyCFunction)Uuid_most_significant_bits,
     METH_NOARGS,
     "  :returns: Most significant 64 bits of the 128 bits Uuid\n"
     "  :rtype: int\n"
     "\n"},
    {"get_least_significant_bits", (PyCFunction)Uuid_least_significant_bits,
     METH_NOARGS,
...
static long Uuid_hash(Uuid *self) {
        ...
        return rd_kafka_Uuid_most_significant_bits(self->cUuid) ^
               rd_kafka_Uuid_least_significant_bits(self->cUuid);
}
...
static int Uuid_init(PyObject *self0, PyObject *args, PyObject *kwargs) {
        Uuid *self         = (Uuid *)self0;
        static char *kws[] = {"most_significant_bits", "least_significant_bits",
                              NULL};
        ...
        self->cUuid =
            rd_kafka_Uuid_new(most_significant_bits, least_significant_bits);
...
static int Uuid_traverse(Uuid *self, visitproc visit, void *arg) {
        return 0;
}
...
PyTypeObject UuidType = {
    PyVarObject_HEAD_INIT(NULL, 0) "cimpl.Uuid", /* tp_name */
    ...
    "\n"
    "  :param long most_significant_bits: Most significant 64 bits of the 128 "
    "bits Uuid.\n"
    "  :param long least_significant_bits: Least significant 64 bits of the "
    "128 bits Uuid.\n"
    "  :rtype: Uuid\n"
    "\n"
    "\n",                        /* tp_doc */
...
static void TopicPartition_setup(TopicPartition *self,
                                 const char *topic,
                                 int partition,
                                 long long offset,
...
static int
TopicPartition_init(PyObject *self, PyObject *args, PyObject *kwargs) {
        const char *topic;
        ...
        static char *kws[] = {"topic",    "partition",    "offset",
                              "metadata", "leader_epoch", NULL};

        if (!PyArg_ParseTupleAndKeywords(args, kwargs, "s|iLsi", kws, &topic,
                                         &partition, &offset, &metadata,
                                         &leader_epoch)) {
                return -1;
        }

        TopicPartition_setup((TopicPartition *)self, topic, partition, offset,
...
static PyObject *
TopicPartition_new(PyTypeObject *type, PyObject *args, PyObject *kwargs) {
        PyObject *self = type->tp_alloc(type, 1);
        return self;
}
...
static int
TopicPartition_traverse(TopicPartition *self, visitproc visit, void *arg) {
        if (self->error)
                Py_VISIT(self->error);
        return 0;
}

static PyObject *TopicPartition_get_leader_epoch(TopicPartition *tp,
                                                 void *closure) {
...
static PyMemberDef TopicPartition_members[] = {
    ...
    {"error", T_OBJECT, offsetof(TopicPartition, error), READONLY,
     ":attribute error: Indicates an error (with :py:class:`KafkaError`) "
     "unless None."},
    {NULL}};

static PyGetSetDef TopicPartition_getters_and_setters[] = {
    {/* name */
     "leader_epoch", (getter)TopicPartition_get_leader_epoch, NULL,
     /* doc */
...
static PyObject *TopicPartition_str0(TopicPartition *self) {
        ...
        snprintf(offset_str, sizeof(offset_str), "%" CFL_PRId64 "",
                 self->offset);
        if (self->leader_epoch >= 0)
                snprintf(leader_epoch_str, sizeof(leader_epoch_str),
                         "%" CFL_PRId32 "", self->leader_epoch);
        else
                snprintf(leader_epoch_str, sizeof(leader_epoch_str), "None");
...
static PyObject *
TopicPartition_richcompare(TopicPartition *self, PyObject *o2, int op) {
...
PyTypeObject TopicPartitionType = {
    PyVarObject_HEAD_INIT(NULL, 0) "cimpl.TopicPartition", /*tp_name*/
 ...
 */
static PyObject *TopicPartition_new0(const char *topic,
                                     int partition,
                                     long long offset,
                                     int32_t leader_epoch,
                                     const char *metadata,
        ...
        self = (TopicPartition *)TopicPartitionType.tp_new(&TopicPartitionType,
                                                           NULL, NULL);

        TopicPartition_setup(self, topic, partition, offset, leader_epoch,
                             metadata, err);
...
PyObject *c_part_to_py(const rd_kafka_topic_partition_t *rktpar) {
        return TopicPartition_new0(
            rktpar->topic, rktpar->partition, rktpar->offset,
            rd_kafka_topic_partition_get_leader_epoch(rktpar), rktpar->metadata,
            rktpar->err);
}

/**
 * @brief Convert C rd_kafka_topic_partition_list_t to Python
 * list(TopicPartition).
 *
...
rd_kafka_topic_partition_list_t *py_to_c_parts(PyObject *plist) {
        ...
        for (i = 0; i < (size_t)PyList_Size(plist); i++) {
                ...
                TopicPartition *tp = (TopicPartition *)PyList_GetItem(plist, i);
                ...
                rktpar = rd_kafka_topic_partition_list_add(c_parts, tp->topic,
                                                           tp->partition);
...
/**
 * @brief Convert C rd_kafka_topic_partition_result_t to Python
 * dict(TopicPartition, KafkaException).
 *
...
PyObject *c_topic_partition_result_to_py_dict(
    ...
    size_t cnt) {
        ...
        for (i = 0; i < cnt; i++) {
                ...
                c_topic_partition = rd_kafka_topic_partition_result_partition(
                    partition_results[i]);
                c_error =
                    rd_kafka_topic_partition_result_error(partition_results[i]);
 ...
 */
static int
py_header_to_c(rd_kafka_headers_t *rd_headers, PyObject *key, PyObject *value) {
        ...
        if (value != Py_None) {
                if (cfl_PyBin(_Check(value))) {
                        ...
                        PyErr_Format(
                            ...
                            ((PyTypeObject *)PyObject_Type(value))->tp_name);
                        Py_DECREF(ks);
                        Py_XDECREF(ks8);
                        return 0;
                }
...
/**
 * @brief Convert Python list[(header_key, header_value),...]) to C
 * rd_kafka_topic_partition_list_t.
 *
...
/**
 * @brief Convert rd_kafka_headers_t to Python list[(header_key,
 * header_value),...])
 *
...
PyObject *c_headers_to_py(rd_kafka_headers_t *headers) {
        ...
        while (!rd_kafka_header_get_all(headers, idx++, &header_key,
                                        &header_value, &header_value_size)) {
...
rd_kafka_consumer_group_metadata_t *py_to_c_cgmd(PyObject *obj) {
        ...
        error = rd_kafka_consumer_group_metadata_read(
            &cgmd, (const void *)buffer, (size_t)size);
        if (error) {
                cfl_PyErr_from_error_destroy(error);
                return NULL;
        }
...
PyObject *c_Node_to_py(const rd_kafka_Node_t *c_node) {
        ...
        Node_type = cfl_PyObject_lookup("confluent_kafka", "Node");
        if (!Node_type) {
                goto err;
        }
...
PyObject *c_Uuid_to_py(const rd_kafka_Uuid_t *c_uuid) {
        ...
        Uuid_type = cfl_PyObject_lookup("confluent_kafka", "Uuid");
        if (!Uuid_type) {
                goto err;
        }
...
/**
 * @brief librdkafka throttle callback triggered by poll() or flush(), triggers
 * the corresponding Python throttle_cb
 */
static void throttle_cb(rd_kafka_t *rk,
        ...
        ThrottleEvent_type =
            cfl_PyObject_lookup("confluent_kafka", "ThrottleEvent");
        ...
        args           = Py_BuildValue("(sid)", broker_name, broker_id,
                                       (double)throttle_time_ms / 1000);
        ...
        result =
            PyObject_CallFunctionObjArgs(h->throttle_cb, throttle_event, NULL);
...
log_cb(const rd_kafka_t *rk, int level, const char *fac, const char *buf) {
        ...
        cs = CallState_get(h);
        result =
            PyObject_CallMethod(h->logger, "log", "issss", level_map[level],
                                "%s [%s] %s", fac, rd_kafka_name(rk), buf);
 ...
 * @returns 1 on success or 0 if an exception was raised.
 */
static int py_extensions_to_c(char **extensions,
                              Py_ssize_t idx,
                              PyObject *key,
                              PyObject *value) {
        PyObject *ks, *ks8, *vo8 = NULL;
...
oauth_cb(rd_kafka_t *rk, const char *oauthbearer_config, void *opaque) {
        ...
        if (!PyArg_ParseTuple(result, "sd|sO!", &token, &expiry, &principal,
                              &PyDict_Type, &extensions)) {
                Py_DECREF(result);
                PyErr_SetString(PyExc_TypeError,
                                "expect returned value from oauth_cb "
                                "to be (token_str, expiry_time[, principal, "
                                "extensions]) tuple");
                goto err;
        }
 ...
 */
static int producer_conf_set_special(Handle *self,
 ...
 * @returns 1 if handled, 0 if unknown, or -1 on failure (exception raised).
 */
static int consumer_conf_set_special(Handle *self,
                                     rd_kafka_conf_t *conf,
                                     const char *name,
                                     PyObject *valobj) {
 ...
 */
static int common_conf_set_special(PyObject *confdict,
                                   rd_kafka_conf_t *conf,
                                   const char *name,
                                   PyObject *vo) {
        const char *v;
        ...
        if (!(vs = cfl_PyObject_Unistr(vo))) {
                PyErr_Format(PyExc_TypeError,
                             "expected configuration property %s "
...
static void common_conf_set_software(rd_kafka_conf_t *conf) {
        ...
        snprintf(version, sizeof(version), "%s-rdkafka-%s", CFL_VERSION_STR,
 ...
 *
 * IMPORTANT: These literals MUST match
...
static int resolve_aws_oauthbearer_marker(PyObject *confdict) {
        ...
        /* 1. Marker present and equals "aws_iam"? */
        ...
        /* 4. Require non-empty sasl.oauthbearer.config. */
        ...
        /* 6. Lazy import; rewrite ImportError friendly. */
        ...
        /* 7. create_handler(cfg_str, ext_str or None) -> Callable */
...
rd_kafka_conf_t *common_conf_setup(rd_kafka_type_t ktype,
        ...
        if (args && PyTuple_Size(args)) {
                if (!PyTuple_Check(args) || PyTuple_Size(args) > 1) {
                        ...
                        return NULL;
                           ...
                           !PyDict_Check(
                               (confdict = PyTuple_GetItem(args, 0)))) {
                        PyErr_SetString(PyExc_TypeError,
        ...
        /*
         * Set debug contexts first to capture all events including plugin
         * loading
         */
        ...
        if ((vo = PyDict_GetItemString(confdict, "plugin.library.paths"))) {
                ...
                if (!common_conf_set_special(
                        confdict, conf, "plugin.library.paths", resolved)) {
                        Py_DECREF(resolved);
                        goto outer_err;
                }
                Py_DECREF(resolved);
        ...
        if ((vo = PyDict_GetItemString(confdict, "default.topic.config"))) {
                /* TODO: uncomment for 1.0 release
                   ...
                   " "set default topic configuration values in the global
                   dict");
                */
        ...
        while (PyDict_Next(confdict, &pos, &ko, &vo)) {
                ...
                if (rd_kafka_conf_set(conf, k, v, errstr, sizeof(errstr)) !=
                    RD_KAFKA_CONF_OK) {
                        cfl_PyErr_Format(RD_KAFKA_RESP_ERR__INVALID_ARG, "%s",
                                         errstr);
                        goto inner_err;
                }
...
PyObject *cfl_PyObject_lookup(const char *modulename, const char *typename) {
        ...
        if (!obj) {
                Py_DECREF(module);
                PyErr_Format(PyExc_TypeError,
                             "No such class/type/object: %s.%s", modulename,
                             typename);
                return NULL;
        }
...
int cfl_PyObject_GetAttr(PyObject *object,
        ...
        if (!o) {
                ...
                PyErr_Format(PyExc_TypeError, "Required attribute .%s missing",
                             attr_name);
                return 0;
        }
        ...
        if (!(allow_None && o == Py_None) && py_type && Py_TYPE(o) != py_type) {
                Py_DECREF(o);
                PyErr_Format(PyExc_TypeError,
                             "Expected .%s to be %s type, not %s", attr_name,
                             py_type->tp_name,
...
int cfl_PyBool_get(PyObject *object, const char *name, int *valp) {
        if (!PyBool_Check(object)) {
                PyErr_Format(PyExc_TypeError,
                             "Expected %s to be bool type, not %s", name,
...
PyObject *cfl_int32_array_to_py_list(const int32_t *arr, size_t cnt) {
        ...
        for (i = 0; i < cnt; i++)
                PyList_SET_ITEM(list, (Py_ssize_t)i, cfl_PyInt_FromInt(arr[i]));
...
const char set_sasl_credentials_doc[] = PyDoc_STR(
    ...
    "  This method will not disconnect existing broker connections that have "
    "been established with the old credentials.\n"
...
static PyObject *libversion(PyObject *self, PyObject *args) {
        return Py_BuildValue("si", rd_kafka_version_str(), rd_kafka_version());
}
...
static PyObject *partitioner_helper(PyObject *args,
        ...
        partition = partitioner_func(NULL, key, (size_t)keylen, partition_count,
                                     NULL, NULL);
...
/**
 * @brief Calculate partition using Murmur2 hash (Java-compatible).
 * Deterministic.
 */
...
#define _PRINT(...)                                                            \
        do {                                                                   \
                ...
                if (_len > sizeof(tmpdoc))                                     \
                        _len = sizeof(tmpdoc) - 1;                             \
                if (dof + _len >= dsize) {                                     \
                        dsize += 2;                                            \
        ...
        _PRINT(
            ...
            "| %-*.*s | %-*.*s |\n"
            "+=%.*s=+=%.*s=+\n",
            _COL1_W, dash, _COL2_W, dash, _COL1_W, _COL1_W, "Constant", _COL2_W,
            _COL2_W, "Description", _COL1_W, eq, _COL2_W, eq);
        ...
        for (i = 0; i < cnt; i++) {
                ...
                _PRINT(
                    "| %-*.*s | %-*.*s |\n"
                    "+-%.*s-+-%.*s-+\n",
                    _COL1_W, _COL1_W, descs[i].name, _COL2_W, _COL2_W,
                    descs[i].desc, _COL1_W, dash, _COL2_W, dash);
        }
...
/* PyEval_InitThreads became deprecated in Python 3.9 and will be removed in
 * Python 3.11. Prior to Python 3.7, this call was required to initialize the
 * GIL. */
        ...
        PyModule_AddIntConstant(m, "TIMESTAMP_LOG_APPEND_TIME",
                                RD_KAFKA_TIMESTAMP_LOG_APPEND_TIME);

        PyModule_AddIntConstant(m, "OFFSET_BEGINNING",

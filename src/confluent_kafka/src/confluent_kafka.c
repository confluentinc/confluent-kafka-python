...
TopicPartition_init(PyObject *self, PyObject *args, PyObject *kwargs) {
        ...
        static char *kws[] = {"topic", "partition", "offset", "metadata", NULL};

        if (!PyArg_ParseTupleAndKeywords(args, kwargs, "s|iLs", kws, &topic,
...
static PyObject *TopicPartition_new0(const char *topic,
        ...
        TopicPartition_setup(self, topic, partition, offset, metadata, err);
...
PyObject *c_parts_to_py(const rd_kafka_topic_partition_list_t *c_parts) {
        ...
        for (i = 0; i < (size_t)c_parts->cnt; i++) {
                ...
                PyList_SET_ITEM(
                    parts, i,
                    TopicPartition_new0(rktpar->topic, rktpar->partition,
                                        rktpar->offset, rktpar->metadata,
                                        rktpar->err));
        }

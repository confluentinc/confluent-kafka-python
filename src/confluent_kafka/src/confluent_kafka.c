...
static PyObject *KafkaError_str0(KafkaError *self)
{
        return cfl_PyUnistr(_FromFormat("KafkaError{%scode=%s,val=%d,str=\"%s\"}",
                                        ...
                                        self->code,
                                        self->str ? self->str : rd_kafka_err2str(self->code)));
}
                                   ...
                                   const char *name, PyObject *vo)
{
        ...
        if (rd_kafka_conf_set(conf, name, v, errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK)
        {
...
// https://docs.python.org/3.9/whatsnew/3.9.html#deprecated -- PyEval_InitThreads() is deprecated since Python 3.9 and slated for removal in Python 3.11.

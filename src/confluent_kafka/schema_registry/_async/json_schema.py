...
async def _resolve_named_schema(
    ...
    if schema.references is not None:
        for ref in schema.references:
            ...
            ref_registry = await _resolve_named_schema(referenced_schema.schema, schema_registry_client, ref_registry)  # type: ignore[arg-type]
...
class AsyncJSONSerializer(AsyncBaseSerializer):
    ...
    async def __serialize(self, obj: object, ctx: Optional[SerializationContext] = None) -> Optional[bytes]:
        ...
        if latest_schema is not None:
            ...
            parsed_schema, ref_registry = await self._get_parsed_schema(latest_schema.schema)  # type: ignore[arg-type]
...
class AsyncJSONDeserializer(AsyncBaseDeserializer):
    ...
    async def __deserialize(self, data: Optional[bytes], ctx: Optional[SerializationContext] = None) -> Optional[bytes]:
        ...
        if self._registry is not None:
            ...
            writer_schema, writer_ref_registry = await self._get_parsed_schema(writer_schema_raw)  # type: ignore[arg-type]
        ...
        if latest_schema is not None and subject is not None:
            ...
            reader_schema, reader_ref_registry = await self._get_parsed_schema(latest_schema.schema)  # type: ignore[arg-type]

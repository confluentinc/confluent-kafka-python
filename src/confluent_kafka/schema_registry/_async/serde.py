...
class AsyncAssociatedNameStrategy:
    ...
    async def _load_subject_name(
        ...
        fallback_type = "TOPIC"  # default fallback
        ...
        if len(associations) > 1:
            ...
            try:
                fallback_strategy = SubjectNameStrategyType(fallback_type)
            except ValueError:
                valid_fallbacks = [e.value for e in SubjectNameStrategyType if e != SubjectNameStrategyType.ASSOCIATED]
                raise ValueError(
                    f"Invalid value for {FALLBACK_SUBJECT_NAME_STRATEGY_TYPE}: {fallback_type}. "

...
instance must not be shared across threads). For `Consumer`, concurrent calls to the same instance are serialized, `ShareConsumer` instead raises `ConcurrentModificationException` (see the

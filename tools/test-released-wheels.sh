...
for version in "${python_versions[@]}"; do
    ...
    if [ "$2" = "test" ]; then
        ...
        install_cmd="PIP_CONFIG_FILE=/dev/null uv pip install --no-cache --no-config --no-deps --prerelease allow --index-url https://test.pypi.org/simple/ confluent_kafka==$1"
    else

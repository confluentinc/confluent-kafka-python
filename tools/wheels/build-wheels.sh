...
export CIBW_SKIP="pp* cp27-* cp35-* cp36-* cp37-macosx_x86_64 *i686 *musllinux* $CIBW_SKIP"
# Run a simple test suite
export CIBW_TEST_EXTRAS="dev"
...
case $OSTYPE in
    linux*)
        os=linux
        # Need to set up env vars (in docker) so that cibuildwheel
        # finds librdkafka.
